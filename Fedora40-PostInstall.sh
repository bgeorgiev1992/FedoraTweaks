...
if prompt_user "Do you want to add Fedora Workstation third-party repositories and RPM Fusion repositories?"; then
        sudo dnf install fedora-workstation-repositories -y || { echo "Failed to install Fedora Workstation repositories"; exit 1; }
        sudo dnf install https://mirrors.rpmfusion.org/free/fedora/rpmfusion-free-release-$(rpm -E %fedora).noarch.rpm -y || { echo "Failed to install RPM Fusion free repository"; exit 1; }
        sudo dnf install https://mirrors.rpmfusion.org/nonfree/fedora/rpmfusion-nonfree-release-$(rpm -E %fedora).noarch.rpm -y || { echo "Failed to install RPM Fusion nonfree repository"; exit 1; }
...
if prompt_user "Do you want to install media codecs?"; then
        sudo dnf config-manager --enable fedora-cisco-openh264 || { echo "Failed to update @core group"; exit 1; }
        ...
        sudo dnf swap 'ffmpeg-free' 'ffmpeg' --allowerasing -y || { echo "Failed to swap ffmpeg"; exit 1; }
        sudo dnf install gstreamer1-plugins-{bad-\*,good-\*,base} gstreamer1-plugin-openh264 gstreamer1-libav --exclude=proj-data-* --exclude=gstreamer1-plugins-bad-free-devel ffmpeg gstreamer-ffmpeg gstreamer1-plugins-bad-free-opencv -y || { echo "Failed to install media codecs"; exit 1; }
        ...
        sudo dnf swap -y mesa-va-drivers mesa-va-drivers-freeworld || { echo "Failed to swap mesa "; exit 1; }
        sudo dnf swap -y mesa-vdpau-drivers mesa-vdpau-drivers-freeworld || { echo "Failed to swap mesa "; exit 1; }
...
if prompt_user "Do you want to install the full Mesa stack with Vulkan support?"; then
        sudo dnf install mesa-dri-drivers mesa-libEGL mesa-libGL mesa-libGLU mesa-libOSMesa mesa-libOpenCL mesa-libd3d mesa-libgbm mesa-libglapi mesa-va-drivers-freeworld mesa-vdpau-drivers-freeworld mesa-vulkan-drivers mesa-libGLw dxvk-native libvkd3d pipewire-plugin-vulkan vulkan-headers vulkan-loader vulkan-tools vulkan-validation-layers gstreamer1-vaapi libvdpau-va-gl libva-utils libva-vdpau-driver libvdpau libvdpau -y || { echo "Failed to install Mesa stak"; exit 1; }
else
...
# 11. Install fonts
if prompt_user "Do you want to install Cascadia, Google Roboto and Microsoft fonts?"; then
        sudo dnf install google-roboto-fonts google-roboto-mono-fonts cabextract xorg-x11-font-utils fontconfig -y || { echo "Failed to install fonts"; exit 1; }
        sudo rpm -i https://downloads.sourceforge.net/project/mscorefonts2/rpms/msttcore-fonts-installer-2.6-1.noarch.rpm || { echo "Failed to install msttcore fonts"; exit 1; }
...
if prompt_user "Do you want to install Google Chrome?"; then
        sudo dnf config-manager --set-enabled google-chrome || { echo "Failed to enable Google Chrome repository"; exit 1; }
        sudo dnf install google-chrome-stable -y || { echo "Failed to install Google Chrome"; exit 1; }
else
